    ...
    <div className={cn("grid auto-cols-fr grid-flow-col gap-2", className)}>
      {tiles.map((t) => (
        <Link
          key={t.href}
          href={t.href}
          className="focus-ring panel grid min-w-0 content-start gap-1 p-3 transition-colors hover:bg-hover"
        >
          <span className="truncate text-xs text-fg-muted">{t.label}</span>
          <span className="truncate text-sm font-medium tabular-nums text-fg">{t.value}</span>
          <span className="truncate text-xs tabular-nums text-fg-muted">{t.sub}</span>
          {/* Under the sub-line, not over it, so the three lines of text sit
              level across tiles that have no bar. */}
          {t.progress !== undefined ? (
            ...
            >
              <span className="block h-full rounded-full bg-accent" style={{ width: `${t.progress}%` }} />
            </span>
          ) : null}
        </Link>
      ))}
